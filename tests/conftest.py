    ...
    if not hasattr(config, "workerinput"):
        ...
        try:
            ...
            if installer.is_toolchain_installed(platform_name, arch):
                print(f"✓ Toolchain already installed for {platform_name}/{arch}")
            else:
                ...
                print(f"✓ Toolchain successfully installed for {platform_name}/{arch}")

            print("=" * 70 + "\n")

        except Exception as e:
            print(f"✗ Failed to pre-install toolchain: {e}")
